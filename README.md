...
- 新增本地兜底规则引擎 `project/rule_engine.py`：当 DeepSeek API 失败时，自动用 EMA/RSI/MACD/ATR 生成保守信号（默认优先 HOLD）。
- 新增通知去抖动：同方向信号在冷却期内不重复弹窗，避免高频噪声提醒。
- 将 tick 的默认成交量占位从 `0.0` 调整为 `1.0`，使K线量能字段可用（仍建议接入真实成交量）。
